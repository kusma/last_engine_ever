...
ILvoid ilImage::Display8b(ILuint xoffset, ILuint yoffset, ILuint* screen, ILuint screenwd) {
	int wd = Width();
	int ht = Height();
	unsigned char* pic = GetData();
	for(int x=0;x<wd;x++) {
		for(int y=0;y<ht;y++) {
			unsigned char c = pic[wd*y+x];
			if(c) screen[screenwd*(y+yoffset)+(x+xoffset)] = (c << 16) | (c << 8) | c;
		}
	}
}
