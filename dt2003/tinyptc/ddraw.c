...
static LRESULT CALLBACK WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	if(userWndProc) result = (*userWndProc)(hWnd,message,wParam,lParam);

    // handle message
    if(!result) switch (message)
    {
        #ifdef __PTC_WINDOWED__
...
HWND ptc_open(char *title,int width,int height,WNDPROC userproc)
{
    #ifdef __PTC_WINDOWED__
    int x;
    int y;
...
int ptc_update(void *buffer)
{
    ...
    // process messages
	// skr-ripped out, we want getmessage. ptc window proc is still called at first, though.
